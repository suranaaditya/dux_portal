...
| File | Purpose |
|---|---|
| `dux_portal/hooks.py` | App hooks: `role_home_page` (System Manager, Accounts User, Purchase User, Stock User -> dux-portal), `app_include_js` (icons, form picker, home button) |
...
| `public/js/portal_home_button.js` | Injects a "Home" button on every desk page linking back to dux-portal |
...
Portal Link `visible_to` field controls who sees each link:
...
- `frappe.get_single()` doesn't reliably load hidden Text fields
...
# Bench console (for DocType creation, data fixes)
bench --site erp.jewonline.in console

# Restart gunicorn workers (needed after Python file changes)
...
- Python file changes require gunicorn worker restart. Workers auto-recycle via `max-requests 5000`, or use `kill -HUP <master_pid>` for immediate reload
- `bench restart` requires sudo on this server. Use `kill -HUP` on gunicorn master or wait for worker recycle

### Deployment gotchas
- `role_home_page` with `"All"` breaks the website root URL — only use specific roles
...
3. **5-mode Visibility** — All, Role, Roles (multi), User, Roles & Users (combined)
4. **Icon Picker** — 148 Lucide icons in 10 categories with search, plain HTML modal
5. **My Items Section** — collapsible section showing items from User Permissions (e.g. companies)
6. **Logo Support** — uploaded logo converted to base64 data URL on the fly, CSS filter options (Make Black/White)
7. **Home Button** — global "Home" button on every desk page
8. **Configurable** — portal_title, footer_text, logo, items section all via Dux Portal Settings
